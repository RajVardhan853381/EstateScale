...
describe("Security Headers", () => {
    it("should enforce strict Content-Security-Policy", async () => {
        ...
        const csp = (globalHeaders?.headers as Record<string, string>[]).find(h => h.key === 'Content-Security-Policy');
        expect(csp?.value).toContain("default-src 'self'");

        const frameOptions = (globalHeaders?.headers as Record<string, string>[]).find(h => h.key === 'X-Frame-Options');
