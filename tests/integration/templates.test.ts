...
describe('Template Service Isolations', () => {
  ...
  it('should prevent cross-tenant duplication', async () => {
    ...
    await expect(TemplateService.duplicateTemplate('org-1', 't1', 'New')).rejects.toThrow(
  ...
  it('should prevent updating Global System templates', async () => {
    ...
    await expect(TemplateService.updateTemplate('org-1', 't1', { name: 'Hack' })).rejects.toThrow(
      'Forbidden: Cannot modify Global System templates'
    );
  });
});
