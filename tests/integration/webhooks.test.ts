...
describe('Twilio Webhooks - Status Updates', () => {
  ...
  beforeAll(async () => {
    ...
    const contact = await prisma.contact.create({
      data: { organizationId: org.id as string, phone: '+1234567890' },
    });

    const conversation = await prisma.conversation.create({
      data: { organizationId: org.id as string, contactId: contact.id },
    });
  ...
  it('rejects request if x-twilio-signature is missing entirely', async () => {
    ...
    const dbMessage = await prisma.message.findUnique({ where: { id: message.id as string } });
