...
export async function GET(req: Request, props: { params: Promise<{ slug: string }> }) {
    try {
        ...
        const templates = await TemplateService.getAvailableTemplates(organization.id, type || undefined);
...
export async function POST(req: Request, props: { params: Promise<{ slug: string }> }) {
    try {
        ...
        if (body.action === "DUPLICATE") {
            ...
            const newTemplate = await TemplateService.duplicateTemplate(organization.id, templateId, newName);
