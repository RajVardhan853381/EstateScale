...
export default function TemplatesSettingsPage(props: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <p className="text-slate-600 mb-8">Manage CRM pipelines, AI Agents, Journeys, and Communication defaults.</p>
          ...
          <div key={t.id} className="p-4 bg-white border rounded-lg flex items-center justify-between shadow-sm">
             <div>
               <div className="flex items-center gap-2 mb-1">
                 ...
                 {t.organizationId && <Badge variant="outline" className="border-green-500 text-green-700">Custom Tenant</Badge>}
               </div>
               <p className="text-sm text-slate-500">Type: {t.type} | {t.description}</p>
             </div>
             <div>
               <Button onClick={() => handleDuplicate(t.id, t.name)} variant="outline">Duplicate to Edit</Button>
             </div>
          </div>
        ))}
