  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
         <Card>
           <CardHeader><CardTitle>Active Journeys</CardTitle></CardHeader>
           <CardContent>
      ...
      <Card>
         <CardHeader><CardTitle>Journey Configurations</CardTitle></CardHeader>
         <CardContent>
            <div className="space-y-4">
               {journeys.map(journey => (
                  <div key={journey.id} className="flex justify-between items-center p-4 border rounded-lg">
                     <div>
                     ...
                     <div className="text-right">
                        <div className="font-medium text-blue-600">{journey._count.enrollments} Enrolled</div>
                        <div className="text-sm text-muted-foreground">v{journey.version}</div>
                     </div>
                  </div>
               ))}
               {journeys.length === 0 && <div className="text-muted-foreground">No active journeys configured yet.</div>}
            </div>
         </CardContent>
      </Card>
    </div>
