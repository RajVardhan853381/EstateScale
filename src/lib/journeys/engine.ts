...
export class JourneyEngine {
  ...
  static async evaluateTrigger(organizationId: string, triggerType: string, leadId: string, eventData: Record<string, unknown> = {}) {
  ...
  static async enrollLead(organizationId: string, journeyId: string, leadId: string, eventData: Record<string, unknown> = {}) {
     try {
         ...
         logger.error({ error: (error as Error).message, journeyId, leadId }, "Failed to enroll lead in journey");
     }
  }

  static async stepCompleted(enrollmentId: string, currentStepId: string, nextStepId?: string, outputData?: Record<string, unknown>) {
      if (!nextStepId) {
