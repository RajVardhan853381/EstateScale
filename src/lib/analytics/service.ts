...
export class AnalyticsService {
  static async getDashboardMetrics(organizationId: string) {
    const [totalLeads, hotLeads, openOpportunities, wonOpportunities, pipelineValueAggr] =
      await Promise.all([
        ...
        ((prisma as any).leadIntelligence as PrismaDynamicMock)?.count({
          where: { organizationId, temperature: 'HOT' },
        }) ?? Promise.resolve(0),
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ((prisma as any).opportunity as PrismaDynamicMock)?.count({
  ...
  static async getLeadFunnel(organizationId: string) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const opportunities =
      (await ((prisma as any).opportunity as PrismaDynamicMock)?.groupBy({
        by: ['stage'],
        where: { organizationId },
        _count: true,
      })) ?? [];
