      ...
      <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          ...
          messages.map((msg) => (
            <div
              key={msg.id}
              className={`flex flex-col max-w-[80%] ${msg.direction === 'OUTBOUND' ? 'ml-auto items-end' : 'mr-auto items-start'}`}
            >
              <div
                className={`px-3 py-2 rounded-lg text-sm ${msg.direction === 'OUTBOUND' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}
              >
                {msg.body}
              </div>
